...
class FishGame {
    ...
    messageFish(message) {
        ...
        if(rod == null) {
            rod = this.getItem(ItemEnum.woodenStick) as Rod;
        }
    ...
    messageCraft(message : DISCORD.Message) {
        var id = parseInt(this.getMessageValue(message, 1));
        if(!Number.isSafeInteger(id)) {
    ...
    getMessageValue(message: DISCORD.Message, index : number) {
    ...
    getPlayer(id) : Player {
        ...
        if(p == undefined) {
            p = new Player(id, 0, Date.now(), new Equipment([], this.getItem(ItemEnum.woodenStick) as Rod));
            this.players.push(p);
        }
        return p;
    }
...
class Equipment {
    ...
    constructor(itemStacks, worstRod : Rod) {
    ...
    recipeRequirements(recipe : ItemStack[]) : ItemStack[] {
    ...
    static fromString(string : string, game : FishGame) : Equipment {
        if(string == null)
            return new Equipment([], game.getItem(ItemEnum.woodenStick) as Rod);
        ...
        return new Equipment(stacks, game.getItem(ItemEnum.woodenStick) as Rod);
    }
}
